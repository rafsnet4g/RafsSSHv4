...
fun_udp1 () {
    ...
    menu
	    ...
	    inst_udp () {
            ...
            wget https://www.dropbox.com/s/clyo6g2ak3mgwx8/badvpn-udpgw -o /dev/null
			wget https://raw.githubusercontent.com/rafsnet4g/RafsSSHv4/main/badvpn/antcrashvpn.sh -o /dev/null
       ...
       fun_udpon2 () {
           screen -dmS udpvpn /bin/antcrashvpn.sh
           [[ $(grep -wc "udpvpn" /etc/autostart) = '0' ]] && {
		       echo -e "ps x | grep 'udpvpn' | grep -v 'grep' && echo 'ON' || screen -dmS udpvpn /bin/badvpn-udpgw --listen-addr 127.0.0.1:7300 --max-clients 10000 --max-connections-for-client 10" >> /etc/autostart
		   } || {
		       sed -i '/udpvpn/d' /etc/autostart
		       echo -e "ps x | grep 'udpvpn' | grep -v 'grep' && echo 'ON' || screen -dmS udpvpn /bin/badvpn-udpgw --listen-addr 127.0.0.1:7300 --max-clients 10000 --max-connections-for-client 10" >> /etc/autostart
		   }
